...
const apiDocente = axios.create({
  baseURL: 'http://localhost:5000/docente',
...
apiDocente.interceptors.request.use((config) => {
...
apiDocente.interceptors.response.use(
  (response) => response,
  async (error) => {
      ...
      if (refreshEnProceso) {
        return new Promise((resolve, reject) => {
          ...
          .then((token) => {
            ...
            return apiDocente(originalRequest)
          })
